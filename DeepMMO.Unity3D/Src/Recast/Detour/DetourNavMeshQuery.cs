...
//////////////////////////////////////////////////////////////////////////////////////////
    ...
    public class dtNavMeshQuery{
        ...
        static bool isectSegSeg(float[] ap, float[] aq, float[] bp, float[] bq, ref float s,ref float t)
        {
	        float[] u = new float[3];
	        float[] v = new float[3];
	        float[] w = new float[3];
								          ...
								          dtPolyRef[] path, ref int pathCount, int maxPath)
        {
	        ...
	        while (!m_openList.empty())
	        {
		        ...
		        for (uint i = bestPoly.firstLink; i != DT_NULL_LINK; i = bestTile.links[i].next)
		        {
			        ...
			        if (neighbourNode.flags == 0)
			        {
				        float[] sa = new float[3];
					    float[] sb = new float[3];
					    getPortalPoints(bestRef, bestPoly, bestTile,
						    neighbourRef, neighbourPoly, neighbourTile, sa, sb);
					    float s = 0.5f;
						float t = 0.5f;
					    if (!isectSegSeg(bestNode.pos,endPos,sa,sb,ref s,ref t))
					    {
						    dtDistancePtSegSqr2D(bestNode.pos, sa, sb, ref t);
					    }
