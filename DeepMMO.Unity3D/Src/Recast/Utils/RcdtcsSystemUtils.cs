...
public static partial class RcdtcsUnityUtils{
	...
	public class SystemHelper {
		...
		public Recast.UnityContext m_ctx = new Recast.UnityContext();
		...
		private byte showProgress = 0;
		
		private Thread m_Thread = null;
		private Action<bool> m_action;

		public void Clear() {
			m_RecastMeshParams = new RecastMeshParams();
		...
		public void AddMesh(Mesh mesh, GameObject transformGao) {
			int vertStart = 0;
			int prevVertCount = 0;
			int newVertCount = mesh.vertexCount;
			...
			if (transformGao != null) {
				for (int i = 0; i < newVertCount; ++i) {
					verts[i] = transformGao.transform.TransformPoint(verts[i]);
				}
			}
			for (int i = 0; i < newVertCount; ++i) {
				int v = vertStart + i * 3;
...
#if UNITY_EDITOR
			if (showProgress == 1)
				EditorUtility.DisplayProgressBar("Exporting navMesh... (" + Mathf.Round(progress * 100) + "%)", "Exporting SceneToNavMesh " , progress);
		...
		public void ComputeSystem(System.Action<bool> action) {
			
			ClearThread();
			m_action = action;
			progress = 0;
			m_Thread = new Thread(CoroutineCompute);
			m_Thread.Start();
		}


		private void CoroutineCompute()
		{
			NewComputeSystem();
			m_action?.Invoke(showProgress > 1);
		}

		private bool NewComputeSystem()
		{
			ClearComputedData();
			showProgress = 1;
			...
			m_cfg.minRegionArea =
				(int) (m_RecastMeshParams.m_regionMinSize *
				       m_RecastMeshParams.m_regionMinSize); // Note: area = size*size
			m_cfg.mergeRegionArea =
				(int) (m_RecastMeshParams.m_regionMergeSize *
				       m_RecastMeshParams.m_regionMergeSize); // Note: area = size*size
			m_cfg.maxVertsPerPoly = (int) m_RecastMeshParams.m_vertsPerPoly;
			m_cfg.detailSampleDist = m_RecastMeshParams.m_detailSampleDist < 0.9f
				? 0
				: m_RecastMeshParams.m_cellSize * m_RecastMeshParams.m_detailSampleDist;
			...
			m_ctx.log(Recast.rcLogCategory.RC_LOG_PROGRESS,
				" - " + m_vertCount / 1000.0f + "K verts, " + m_triCount / 1000.0f + "K tris");
			...
			if (!Recast.rcCreateHeightfield(m_ctx, m_solid, m_cfg.width, m_cfg.height, m_cfg.bmin, m_cfg.bmax, m_cfg.cs,
				m_cfg.ch))
			{
			...
			if (m_triareas == null)
			{
				m_ctx.log(Recast.rcLogCategory.RC_LOG_ERROR,
					"buildNavigation: Out of memory 'm_triareas' (" + m_triCount + ").");
				return false;
			}
			...
			Recast.rcMarkWalkableTriangles(m_ctx, m_cfg.walkableSlopeAngle, m_verts, m_vertCount, m_tris, m_triCount,
				m_triareas);
			Recast.rcRasterizeTriangles(m_ctx, m_verts, m_vertCount, m_tris, m_triareas, m_triCount, m_solid,
				m_cfg.walkableClimb);
			...
			if (!Recast.rcBuildPolyMeshDetail(m_ctx, m_pmesh, m_chf, m_cfg.detailSampleDist, m_cfg.detailSampleMaxError,
				m_dmesh))
			{
			...
			{
				m_ctx.log(Recast.rcLogCategory.RC_LOG_WARNING,
					"Detour does not support more than " + Detour.DT_VERTS_PER_POLYGON +
					" verts per polygon. A navmesh has not been generated.");
			}
			...
			m_ctx.logBuildTimes();
			m_ctx.log(Recast.rcLogCategory.RC_LOG_PROGRESS,
				">> Polymesh: " + m_pmesh.nverts + " vertices  " + m_pmesh.npolys + " polygons");
		...
		public Tuple<bool,Vector3> GetClosestPointOnNavMesh(Vector3 pos,float distance,Detour.dtQueryFilter filter = null)
		{
			...
			var ret = m_navQuery.findNearestPoly(startpos, extents, filter, ref startRef, ref res);
